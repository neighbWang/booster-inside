...
从图中可以看出，在 `onPause()` 的时候，会调用 [QueuedWork.waitToFinish()](https://github.com/aosp-mirror/platform_frameworks_base/blob/master/core/java/android/app/QueuedWork.java#L154)，而 `waitToFinish()` 就是将之前 `Editor.apply()` 往 `QueuedWork` 队列中追加的 `Runnable` 在主线程同步执行，如下代码所示：
